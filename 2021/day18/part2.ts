...
function test(line1: string, line2: string) {
  const nodel = parseLine(line1);
  const noder = parseLine(line2);
  const newRoot = (nodel.parent = noder.parent = node(undefined));
  newRoot.l = nodel;
  newRoot.r = noder;
  process(newRoot);
  bigm = Math.max(bigm, magnitude(newRoot));
}
