...
function calories(ingredients: Ingredient[], mult: number[]) {
  return product(
    ingredients.map((i) => i.calories),
    mult
  ).reduce((acc, x) => acc + x);
}
...
const scores = combinations.filter((c) => calories(ingredients, c) === 500).map((c) => score(ingredients, c));
