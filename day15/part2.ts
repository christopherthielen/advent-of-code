...
const increment = (initial: number, incrementBy: number) => {
  const val = initial + incrementBy;
  return val > 9 ? val - 9 : val;
};

const hslice1 = grid1.map((initial) => {
  const segment2 = initial.map((x) => increment(x, 1));
  const segment3 = initial.map((x) => increment(x, 2));
  const segment4 = initial.map((x) => increment(x, 3));
  const segment5 = initial.map((x) => increment(x, 4));
  return initial.concat(segment2).concat(segment3).concat(segment4).concat(segment5);
});

const hslice2 = hslice1.map((line) => line.map((x) => increment(x, 1)));
const hslice3 = hslice1.map((line) => line.map((x) => increment(x, 2)));
const hslice4 = hslice1.map((line) => line.map((x) => increment(x, 3)));
const hslice5 = hslice1.map((line) => line.map((x) => increment(x, 4)));

const grid = hslice1.concat(hslice2).concat(hslice3).concat(hslice4).concat(hslice5);
console.log(grid);
