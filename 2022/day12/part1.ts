...
start.val.distance = 0;
const visit = (node: Node) => {
  node.neighbors(Item.DIRECTIONS.STRAIGHT).forEach((neighbor) => {
    const canTraverse = node.val.height + 1 >= neighbor.val.height;
...
console.log(grid.toString((item) => `[${pad(`${item.val.height}:${item.val.distance}`, 7)}]`));

console.log(dest.val.distance);
